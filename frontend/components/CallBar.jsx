...
export default function CallBar({ lead, contact, onClose, onCallLogged }) {
  ...
  const startCall = async () => {
    ...
    try {
      ...
      c.on("accept", (conn) => { setStatus("in_call"); startTimer(); setCallSid(conn?.parameters?.CallSid || null); });
  ...
  const endCall = () => { call?.disconnect(); device?.disconnectAll(); setStatus("ended"); stopTimer(); };
  const toggleMute = () => { if (!call) return; call.mute(!muted); setMuted(!muted); };
  ...
  const logAndClose = async () => {
    ...
    try {
      await fetch(`${BASE}/calls/log`, {
        ...
        body: JSON.stringify({ lead_id: lead.id, contact_id: contact?.id || null, twilio_call_sid: callSid, direction: "outbound", duration_seconds: seconds, outcome, notes, called_at: new Date().toISOString() }),
      });
      onCallLogged?.();
      onClose();
    } catch (e) { alert("Failed to log call: " + e.message); }
    finally { setSaving(false); }
  };

  const statusColors = { idle: C.muted, connecting: C.callback, ringing: C.callback, in_call: C.confirmed, ended: C.dead, error: C.dead };
  const statusLabels = { idle: device ? "Ready" : "Loading...", connecting: "Connecting...", ringing: "Ringing...", in_call: `In call  ${fmt(seconds)}`, ended: `Call ended  ${fmt(seconds)}`, error: "Setup error — check .env" };
    ...
    <div style={{ position: "fixed", bottom: 24, left: "50%", transform: "translateX(-50%)", zIndex: 500, background: C.surface, border: `1px solid ${C.border}`, borderRadius: 16, padding: "20px 28px", boxShadow: "0 16px 48px rgba(0,0,0,0.18)", minWidth: 480, display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div>
          ...
          <div style={{ ...T.mono, fontSize: 12, color: C.muted, marginTop: 2 }}>{contact?.name ? `${contact.name} · ` : ""}{phoneNumber || "No phone number"}</div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          <span style={{ ...T.mono, fontSize: 11, color: statusColors[status], fontWeight: 500 }}>{statusLabels[status]}</span>
      ...
      {status !== "ended" ? (
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          ...
          {(status === "connecting" || status === "ringing") && (
            <button onClick={endCall} style={{ flex: 1, padding: "12px", borderRadius: 10, border: "none", background: C.dead, color: "#FFF", ...T.body, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>Cancel</button>
          )}
          {status === "in_call" && (
            <>
              <button onClick={toggleMute} style={{ padding: "12px 20px", borderRadius: 10, border: `1px solid ${C.border}`, background: muted ? C.callbackBg : C.subtle, color: muted ? C.callback : C.muted, ...T.body, fontSize: 13, cursor: "pointer" }}>
                {muted ? "🔇 Muted" : "🎤 Mute"}
              </button>
              <button onClick={endCall} style={{ flex: 1, padding: "12px", borderRadius: 10, border: "none", background: "rgba(220,50,50,0.9)", color: "#FFF", ...T.body, fontSize: 14, fontWeight: 600, cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Notes about the call..." rows={2}
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button onClick={onClose} style={{ ...T.body, fontSize: 13, padding: "8px 16px", borderRadius: 8, border: `1px solid ${C.border}`, background: "transparent", color: C.muted, cursor: "pointer" }}>Skip</button>
            <button onClick={logAndClose} disabled={saving} style={{ flex: 1, padding: "9px", borderRadius: 8, border: "none", background: C.accent, color: "#FFF", ...T.body, fontSize: 13, fontWeight: 500, cursor: "pointer" }}>
